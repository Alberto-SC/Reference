 ...
 * Date: 2019-10-30
 * Description: Binary tree based in values instead of ranges like segment tree, thah alows 
 * compute queries in a range like , kth smallest element in a range [l,r], other queries in the code.
 * Time: O( \log N).
 * Status: Tested
 * Problems: 
 *  \begin{itemize}
 ...
 *      \item [SPOJ MKTHNUM]
 *      \item [Codeforces 1837F - Editorial for Two] (kth Sum in range)
 *  \end{itemize}
 * Considerations: 
 *  \begin{itemize}
*       \setlength\itemsep{0.05em}
 *      \item compression if the elements are to big.
 *      \item Array passed is modified
 *  \end{itemize}
 * Usage: 
 *       wavelet wt(Array,Max_element+1);
...
struct wavelet{
    ...
    /* 
        ...
        the problem tested has a k up to $(j-i)/5$ and the complexity has  $O(5 \log n)$
    */
...
// exclude-from-here 
/* Extendend Usage: Solution of problem [SPOJ - MKTHNUM] */
