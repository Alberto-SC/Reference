...
namespace generator {
  ...
  vector<int> gen_unique_array(int len = 0, int minRange = 0, int maxRange = inf){
    assert(len >= 0 and len <= 5e6);
    set<int> a;
    while(a.size()<len){
        a.insert(rand(minRange,maxRange));
    }
    vector<int> array(a.begin(),a.end());
    return array;
  }
...
void generate_test() {
    int n = rand(1, 100000);
    int m = rand(1,1000000/n);
    cout << n <<" "<<m<<endl;
    for(int i = 0;i<n;i++){
	    cout << gen_array(m,1,100);
    }
}
