...
namespace generator {
  ...
  string gen_string_combined(int len = 0,int l = 1, int r = 26) {
    assert(len >= 0 && len <= 5e6);
    string str(len,'A');
    for (char &ch: str) {
      ch = (rand(0,1)? 'A':'a') +rand(l, r) - 1;
    }
    return str;
  }
...
void generate_test() {
    int l = rand(1,1000000);
    int r = rand(l,1000000);
    cout<<l<<" "<<r<<endl;
}
...
signed main() {
  ...
  int t = 1;
  #ifndef SINGLE_TEST
    t = rand(max_tests, max_tests), cout << t << '\n';
  #endif
  while (t--) {
    generate_test();
  }
