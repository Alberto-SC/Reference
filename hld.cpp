...
void make_tree(int cur, int s, int e) {
	...
	int c1 = (cur<<1) + 1, c2 = c1 + 1, m = (s+e)>>1;
	make_tree(c1, s, m);
	make_tree(c2, m+1, e);
...
int query_tree(int cur, int s, int e, int S, int E) {
    ...
    if(E <= m)
        return query_tree(c1, s, m, S, E);
    else if(S > m)
        return query_tree(c2, m+1, e, S, E);
    else {
        return max(query_tree(c1, s, m, S, m),
                query_tree(c2, m+1, e, m+1, E));
    }
}
...
int query_up(int u, int v) {
	...
	while(1) {
		...
		if(uchain == vchain) {
			...
			int res = query_tree(0, 0, ptr-1, posInBase[v], posInBase[u]);
			// Above is call to segment tree query function
			if(res > ans) ans = res; // Update answer
			break;
		}
		int res = query_tree(0, 0, ptr-1, posInBase[chainHead[uchain]], posInBase[u]);
...
int main() {
	...
	while(t--) {
		...
		make_tree(0, 0, ptr-1); // We use baseArray and construct the needed segment tree
